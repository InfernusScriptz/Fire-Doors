		...
		if d then
			if d:IsA("BasePart") then
				...
				if d.Name == "Shade" then
					coroutine.wrap(function()
						repeat
							d.Anchored = bools.NoHalt
							rs(1)
						until closed or not d
					end)()
				end
			end
			...
			if d:IsA("Model") and not d:IsA("Tool") then
				if d.Name == "TriggerEventCollision" then
					coroutine.wrap(function()
						...
						if workspace.CurrentRooms:FindFirstChild("50") and d:IsDescendantOf(workspace.CurrentRooms["50"]) then
							return
						end
						repeat
